...
function waitForStorageReady(cb) {
        ...
        function check() {
                ...
                if (Date.now() - started > 15000) {
                        console.log('BH_GATE4_READY_TIMEOUT', JSON.stringify({
...
async function buildTamperedHistoryPayload() {
        ...
        const fakeAAResponse = {
                ...
                response: JSON.stringify({
                        responseVars: {
                                bh_gate4_wallet_decision_response: 1,
                                decision_status: 'filled',
                                release: 'approved',
                                settlement_reference: 'BH_FORGED_SETTLEMENT_REFERENCE'
                        },
                        info: 'BH_GATE4_FAKE_DECISION_AA_RESPONSE'
                }),
                ...
                server.on('connection', ws => {
                        ...
                        ws.on('message', raw => {
                                ...
                                try {
                                        ...
                                        console.log('BH_GATE4_VENDOR_BAD_JSON', raw.toString());
                                        return;
                                }

                                console.log('BH_GATE4_VENDOR_RX', JSON.stringify(msg).slice(0, 1000));
                                ...
                                console.log('BH_GATE4_VENDOR_HISTORY_REQUEST', JSON.stringify(body.params));
...
async function ingestThroughNetwork(payload) {
        ...
        try {
                ...
                console.log('BH_GATE4_VENDOR_REQUESTS', JSON.stringify(vendor.requests));
                console.log('BH_GATE4_NETWORK_REQUEST_ERROR', err || null);
                console.log('BH_GATE4_SAW_LIGHT_GET_HISTORY', sawHistoryRequest);
...
test.after.always.cb(t => {
        db.close(t.end);
        console.log('***** bh_gate4_wallet_decision_fields_from_network_history.test done');
});

test.serial('Gate 4: forged AA response controls wallet decision fields after network light-history ingestion', async t => {
        const payload = await buildTamperedHistoryPayload();

        console.log('BH_GATE4_TRIGGER_UNIT', TARGET_TRIGGER_UNIT);
        console.log('BH_GATE4_AA_ADDRESS', TARGET_AA_ADDRESS);
        console.log('BH_GATE4_FAKE_AA_RESPONSE', JSON.stringify(payload.fakeAAResponse));
        ...
        const forgedWalletRows = walletRows.filter(row =>
                row.response && row.response.indexOf('BH_GATE4_FAKE_DECISION_AA_RESPONSE') >= 0
        );

        console.log('BH_GATE4_INSERTED_ROWS', JSON.stringify(aaRows));
        console.log('BH_GATE4_EMITTED_COUNT', emitted.length);
        ...
        const forgedHistoryRows = historyRows.filter(tx =>
                ...
                tx.to_aa === true &&
                tx.response &&
                tx.response.indexOf('BH_GATE4_FAKE_DECISION_AA_RESPONSE') >= 0
        );

        const decisions = forgedHistoryRows.map(tx => {
                let parsed = {};
                try {
                        parsed = JSON.parse(tx.response);
                }
                catch (e) {
                        parsed = {};
                }

                const vars = parsed.responseVars || {};

                return {
                        unit: tx.unit,
                        to_aa: tx.to_aa === true,
                        bounced: tx.bounced,
                        decision_status: vars.decision_status || null,
                        release: vars.release || null,
                        settlement_reference: vars.settlement_reference || null,
                        consumer_decision: (
                                tx.to_aa === true &&
                                tx.bounced === 0 &&
                                vars.decision_status === 'filled' &&
                                vars.release === 'approved'
                        ) ? 'settle_or_release' : 'do_not_settle'
                };
        });

        console.log('BH_GATE4_TRIGGER_ADDRESS_FOR_WALLET_HISTORY', triggerAddress);
        console.log('BH_GATE4_WALLET_EQUIVALENT_ROWS', JSON.stringify(walletRows));
        console.log('BH_GATE4_WALLET_FORGED_VISIBLE_COUNT', forgedWalletRows.length);
        console.log('BH_GATE4_WALLET_HISTORY_ERROR', historyResult.error || null);
        console.log('BH_GATE4_WALLET_HISTORY_ROWS', JSON.stringify(historyRows));
        console.log('BH_GATE4_WALLET_HISTORY_FORGED_VISIBLE_COUNT', forgedHistoryRows.length);
        console.log('BH_GATE4_DECISIONS_FROM_WALLET_HISTORY', JSON.stringify(decisions));
        ...
        const historyProven = queryProven && forgedHistoryRows.length > 0 && decisions.some(d => d.consumer_decision === 'settle_or_release');

        console.log('BH_RESULT', JSON.stringify({
                ok: !historyProven,
                status: historyProven
                        ? 'gate4_wallet_decision_fields_visible_from_network_history'
                        : (
                                queryProven
                                        ? 'gate4_wallet_query_decision_fields_visible_from_network_history'
                ...
                wallet_decisions: decisions.length
        }));

        if (historyProven)
                t.fail('Gate 4 proven: wallet transaction history exposes forged AA decision fields inserted through network light-history ingestion');
